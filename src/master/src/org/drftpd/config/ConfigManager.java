 ...
 * the startup process and *MUST* be an extension of the master extension-point
 * "ConfigHandler".<br>
 * No hard coding is needed to handle new directives, simply create a new
 * extension.
 * 
 * @author fr0w
 * @version $Id$
...
public class ConfigManager implements ConfigInterface {
	...
	private static final Key<Hashtable<String, ArrayList<PathPermission>>> PATHPERMS = new Key<>(ConfigManager.class,
			"pathPerms");
	...
	/**
	 * Reload all VFSPermHandlers and ConfigHandlers. Also re-read the config files.
	 */
	public void reload() {
		loadVFSPermissions();
		loadConfigHandlers();
	...
	/**
	 * Returns the KeyedMap that allow dynamic and persistent storage of the loaded
	 * permissions.<br>
	...
	private void loadConfigHandlers() {
		...
		try {
			List<PluginObjectContainer<ConfigHandler>> loadedDirectives = CommonPluginUtils.getPluginObjectsInContainer(
					this, "master", "ConfigHandler", "Class", "Method",
	...
	/**
	 * Reads 'conf/perms.conf' handling what can be handled, ignoring what's does
	 * not have an available handler.
	 */
	...
	public void addPermission(String directive, Permission permission) {
		...
		if (alreadyExists) {
			...
			logger.info("The directive '" + directive + "' is already on the permission map, check out your "
					+ permsFile.getName());
			return;
		}
